...
switch($action) {
	...
	default:
		if($submit) {
			(isset($typeids) && is_array($typeids) && $typeids) or message($L['mail_msg_choose'], $MOD['linkurl'].'wapmail.php');
			...
			dmsg($L['mail_msg_update'], 'wapmail.php');
		} else {
			$mytypeids = array();
			if($r) {
...
include template('mail', 'memberwap');
