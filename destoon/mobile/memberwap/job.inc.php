...
switch($action) {
	case 'add':
		if($MG['resume_limit'] && $limit_used >= $MG['resume_limit']) dalert(lang($L['info_limit'], array($MG['resume_limit'], $limit_used)), $_userid ? $DT['file_my'].'?mid='.$mid : $DT['file_my']);
		if($MG['day_limit']) {
			$today = $today_endtime - 86400;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $sql AND addtime>$today");
			if($r && $r['num'] >= $MG['day_limit']) dalert(lang($L['day_limit'], array($MG['day_limit'])), $_userid ? $DT['file_my'].'?mid='.$mid : $DT['file_my']);
		}
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if($_userid) {
					set_cookie('dmsg', $msg);
					$forward = $DT['file_my'].'?mid='.$mid.'&resume=1&status='.$post['status'];
...
include template('my_resume', 'memberwap');
...
switch($action) {
	case 'add':
		if($MG['job_limit'] && $limit_used >= $MG['job_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $DT['file_my'].'?mid='.$mid : $DT['file_my']);
		if($MG['day_limit']) {
			$today = $today_endtime - 86400;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $sql AND addtime>$today");
			if($r && $r['num'] >= $MG['day_limit']) dalert(lang($L['day_limit'], array($MG['day_limit'])), $_userid ? $DT['file_my'].'?mid='.$mid : $DT['file_my']);
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if($_userid) {
					set_cookie('dmsg', $msg);
					$forward = $DT['file_my'].'?mid='.$mid.'&status='.$post['status'];
					$msg = '';
				} else {
					$forward = $DT['file_my'].'?mid='.$mid.'&action=add';
				}
...
include template('my_job', 'memberwap');
