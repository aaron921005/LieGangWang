...
switch($action) {
	case 'add':
		if($MG['video_limit'] && $limit_used >= $MG['video_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $DT['file_my'].'?mid='.$mid : $DT['file_my']);
		if($MG['day_limit']) {
...
include template($MOD['template_my'] ? $MOD['template_my'] : 'my_'.$module, 'memberwap');
?>