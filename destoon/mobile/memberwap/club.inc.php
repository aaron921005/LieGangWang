...
if(in_array($job, array('group', 'reply', 'join', 'fans', 'manage'))) {
	require DT_ROOT.'/mobile/memberwap/club_'.$job.'.inc.php';
} else {
	$job = '';
	...
	switch($action) {
		case 'add':
			...
			if($MG['club_limit'] && $limit_used >= $MG['club_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MUSER.'?mid='.$mid : $MUSER);
			...
			if($MG['day_limit']) {
				...
				if($r && $r['num'] >= $MG['day_limit']) dalert(lang($L['day_limit'], array($MG['day_limit'])), $_userid ? $MUSER.'?mid='.$mid : $MUSER);
			}
				...
				if($do->pass($post)) {
					...
					if($post['status'] == 3) {
						...
						if($_userid) {
							...
							$forward = $MUSER.'?mid='.$mid.'&action=add';
							$msg = $L['success_check'];
						}
					}
