...
switch($action) {
	case 'add':
		if($MG['know_limit'] && $limit_used >= $MG['know_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $DT['file_my'].'?mid='.$mid : $DT['file_my']);
		if($MG['day_limit']) {
...
include template('my_'.$module, 'memberwap');
?>