...
switch($action) {
	case 'add':
		...
		if($MG['club_group_limit'] && $limit_used >= $MG['club_group_limit']) dalert(lang($L['info_limit'], array($MG['club_group_limit'], $limit_used)), $MUSER.'?mid='.$mid.'&job='.$job);
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				$js = '';
				set_cookie('dmsg', $msg);
				$forward = $MUSER.'?mid='.$mid.'&job='.$job.'&status='.$post['status'];
				$msg = '';
