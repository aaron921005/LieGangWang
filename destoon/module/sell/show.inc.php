...
$xhmember = userinfo($item['username']);
$COM = userinfo($username);
$comname = urlencode($item['company']);
$uaddress = urlencode($item['address']);
$uarea = urlencode($item[areaname]);
$userid = get_user($item['username']);
$MTYPE = get_type('product-'.$userid);
$catids = explode(',', substr($xhmember['catid'], 1, -1));
//xiaohei-20170415 - E
...
if(check_group($_groupid, $MOD['group_contact'])) {
	if($fee) {
		...
		if($member) {
			...
			$db->get_one("UPDATE {$DT_PRE}sell_5 SET favoritecm=favoritecm+1 WHERe itemid=$itemid");
		}
	}
} else {
