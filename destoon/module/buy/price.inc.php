...
$item = $db->get_one("SELECT title,tag,linkurl,totime,thumb,username,truename,telephone,mobile,qq,company,vip,status FROM {$table} WHERE itemid=$itemid");
...
if($submit) {
	...
	if(send_message($item['username'], $title, $message, 2, $_username)) {
		$db->get_one("UPDATE {$DT_PRE}buy_6 SET pricenums=pricenums+1 WHERE itemid=$itemid");
